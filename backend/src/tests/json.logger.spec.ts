...
describe('JsonLogger', () => {
  it('Create log-file in the specified directory', () => {
    ...
    new JsonLogger(dirName, fileName, ['log', 'error']);
  ...
  it('Write logs with different levels', () => {
    const expectedFilePath = path.join(dirName, fileName);
    const logger = new JsonLogger(dirName, fileName, [
      'log',
      'error',
      'warn',
      'debug',
    ...
    expect(content).toMatch(
      new RegExp(
        `{"timestamp":".*","level":"LOG","message":"Test log message"}`,
      ),
    );
    expect(content).toMatch(
      new RegExp(
        `{"timestamp":".*","level":"ERROR","message":"Test error message","additionalInfo":\\[.*\\]}`,
      ),
    );
    expect(content).toMatch(
      new RegExp(
        `{"timestamp":".*","level":"WARN","message":"Test warning message"}`,
      ),
    );
    expect(content).toMatch(
      new RegExp(
        `{"timestamp":".*","level":"DEBUG","message":"Test debug message"}`,
      ),
    );
  });

  it('Filter out messages based on allowed log levels', () => {
    const logger = new JsonLogger(dirName, fileName, ['log', 'error']);
    const expectedFilePath = path.join(dirName, fileName);
